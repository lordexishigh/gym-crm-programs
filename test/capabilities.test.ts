...
describe("capabilityGaps", () => {
  it("excludes optional capabilities so the list stays worth reading", () => {
    // Payments and AI are deliberately off on this product; reporting them as
    // problems trains people to ignore the notice that also carries email.
    const ids = capabilityGaps().map((c) => c.id);
    expect(ids).not.toContain("payments");
    expect(ids).not.toContain("ai");
  });

  it("includes email and the scheduler, the two silent failures", () => {
    const ids = capabilityGaps().map((c) => c.id);
    expect(ids).toContain("email");
    expect(ids).toContain("scheduler");
  });

  it("classifies email and scheduler as degraded, not required", () => {
