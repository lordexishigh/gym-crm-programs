/**
 * In-process sliding-window rate limiter (security hardening).
 *
 * Built for the credential-brute-force guard on the two sign-in Server Actions
 * (see lib/auth/login-throttle.ts), which had no throttle at all: an attacker
 * could post the /login or /portal/login form as fast as the auth service
 * would answer.
 *
 * WHY A SLIDING WINDOW rather than a fixed one: a fixed window lets a caller
 * spend its whole budget at the end of window N and again at the start of
 * window N+1 — a 2x burst across the boundary, which is exactly the shape a
 * password sprayer produces. Keeping the individual hit timestamps costs a
 * small array per key and removes that edge entirely.
 *
 * WHY IN-PROCESS: the project has no Redis/Upstash dependency, and adding one
 * to gate a login form would be the largest new piece of infrastructure in the
 * stack. The honest trade-off, stated plainly because it bounds what this can
 * promise: on a multi-instance/serverless deployment each instance keeps its
 * own counters, so the effective global limit is (instances x limit) per
 * window. That still converts an unbounded brute force into a bounded one —
 * the property that actually matters — and it is a strict improvement over no
 * limit. The `RateLimiterStore` shape below is deliberately narrow so a shared
 * backend can be dropped in later without touching a single call site.
 *
...
export class SlidingWindowRateLimiter implements RateLimiterStore {
   ...
   *
   * A REJECTED ATTEMPT IS NOT RECORDED. That is the important detail: if
   * blocked attempts were also written to the window, a caller hammering the
   * endpoint would keep pushing its own oldest-hit forward and could never
   * come back — a permanent lockout triggered by the attacker, on a key (the
   * client IP) that a legitimate user may share via NAT. Not recording them
   * means the window drains on schedule and the sustained rate is exactly
   * `limit` per `windowMs`, which is the throttle we wanted.
   */
  consume(key: string): RateLimitDecision {
    ...
    if (recent.length >= this.rule.limit) {
      // Keep the pruned array so the retry estimate stays accurate, and
      // refresh recency so this key is not the one evicted under pressure.
   ...
   * per-key prune in `consume` already keeps individual entries correct, so
   * this only reclaims keys that stopped being asked about, and doing it on
   * every call would make each request O(tracked keys).
   */
 ...
 * platform-generated headers come first because the proxy writes them itself
 * and a client cannot forge them: Vercel sets `x-vercel-forwarded-for` and
 * `x-real-ip`, Cloudflare sets `cf-connecting-ip`. `x-forwarded-for` is the
 * last resort (and behind Vercel it is overwritten by the edge anyway).
 */
const IP_HEADERS = [
